...
if __name__ == "__main__":
    ...
    parser.add_argument("input_path", help="Path with raw image for face alignment")
    parser.add_argument("output_path", help="Directory for storing aligned image")
    parser.add_argument("--face_landmarks_model_path", default="./shape_predictor_68_face_landmarks.dat", help="Path to face landmark model")
    ...
    RAW_IMAGE_PATH = args.input_path
    ALIGNED_IMAGES_DIR = args.output_path
    LANDMARKS_MODEL_PATH = args.face_landmarks_model_path

    for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(RAW_IMAGE_PATH), start=1):
        try:
            image_align(RAW_IMAGE_PATH, ALIGNED_IMAGES_DIR, face_landmarks, output_size=args.output_size, x_scale=args.x_scale, y_scale=args.y_scale, em_scale=args.em_scale, alpha=args.use_alpha)
